...
export function ReportSummaryCards({
  ...
  const validationStatus = isAnySimulationActive
    ? anomalyCount === 0
      ? {
          label: "Tervalidasi (Live)",
          color: "text-brand-accent-teal",
          icon: CheckCircle2,
        }
      : {
          label: `${anomalyCount} Anomali Ditandai`,
          color: "text-brand-accent-amber",
          icon: AlertTriangle,
        }
    : {
        label: "Siap untuk Pengajuan SRUK (Q1)",
        color: "text-brand-accent-teal",
        icon: CheckCircle2,
      };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="bg-surface-card rounded-2xl p-6 border border-surface-hairline flex flex-col justify-between min-h-[140px]">
        ...
        {isAnySimulationActive && (
          <StatusBadge
            label={`+${(liveSessionKwh / 1000).toFixed(4)} MWh (Sesi Live)`}
            variant="teal"
            pulse
          />
        )}
      ...
      <div className="bg-surface-card rounded-2xl p-6 border border-surface-hairline flex flex-col justify-between min-h-[140px]">
        ...
        {isAnySimulationActive && (
          <StatusBadge
            label={`Faktor CO₂e: ${co2Factor} kgCO2e/kWh`}
            variant="amber"
            pulse
          />
        )}
      ...
      <div className="bg-surface-dark rounded-2xl p-6 text-text-on-dark flex flex-col justify-between min-h-[140px]">
        <div className="flex items-center gap-2 mb-2">
          <validationStatus.icon
            className={`w-4 h-4 ${validationStatus.color}`}
          />
          <p
            className={`text-[10px] uppercase font-bold ${validationStatus.color} tracking-widest`}
          >
            {anomalyCount > 0 && isAnySimulationActive
              ? "Perlu Ditinjau"
              : "Status Tervalidasi"}
          </p>
        </div>
